...
## to enumerate the HGNC stable transcript ID, split by ensg_gene_name_combined
## assign a number to each unique ENSG that has ever been assigned to the gene symbol by order of release. Duplicate ENSG IDs are delineated by ENSG numerical order.
## number each unique ENST ID according to their order for each ENSG.
...
#   .x = list_tibble_all_ENST_IDs_split_by_gene_name %>% purrr::map(~.x$gene_id %>% unique),
#   .f = function(a1) {
#     
#     # DEBUG ###
#     # a1 <- list_tibble_all_ENST_IDs_split_by_gene_name %>% purrr::map(~.x$gene_id %>% unique) %>% .[[1]]
#     ###########
#     
...
tibble_HGNC_stable_transcript_IDs <- furrr::future_imap(
  .x = list_tibble_all_ENST_IDs_split_by_gene_name,
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a2 <- list_tibble_all_ENST_IDs_split_by_gene_name[[17819]]
    # a1 <- list_tibble_all_ENST_IDs_split_by_gene_name[[76616]]
    # a1 <- list_tibble_all_ENST_IDs_split_by_gene_name[[24]]
    # a1 <- list_tibble_all_ENST_IDs_split_by_gene_name[[32]]
    # a1 <- list_tibble_all_ENST_IDs_split_by_gene_name[[1]]
    ...
    tibble_ENSG_earliest_release_joined <- a1 %>%
    ...
    # table join
    tibble_with_ENSG_historical_join_order <- dplyr::left_join(a1, tibble_ENSG_earliest_release_joined)
    
    # finally add in the stable transcript number
    tibble_hgnc_stable_transcript_id <- tibble_with_ENSG_historical_join_order %>% 
      ungroup() %>% 
      group_split(gene_id) %>%
      purrr::imap(
        .x = .,
        .f = function(b1, b2) {
          ...
          # DEBUG ###
          # b1 <- tibble_with_ENSG_historical_join_order %>% ungroup() %>% group_split(gene_id) %>% .[[1]]
          ###########
          
          # print(b2)
          
          output <- b1 %>% dplyr::arrange(transcript_id)
      ...
      dplyr::ungroup() %>%
      dplyr::mutate("hgnc_stable_transcript_ID" = paste(ensg_gene_name_combined, "-", ENSG_historical_join_order, "enst", stable_transcript_number, ".", transcript_version, sep = "") %>% gsub(pattern = "\\-1enst", replacement = "-enst"))
...
# fetch the table join data for each gene tree
# list_ENSG_ID_table_join_per_gene_tree <- purrr::imap(
#   .x = list_tibble_all_ENSP_IDs_split_by_gene_name %>% purrr::map(~.x$gene_id %>% unique),
#   .f = function(a1) {
#     
#     # DEBUG ###
#     # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name %>% purrr::map(~.x$gene_id %>% unique) %>% .[[1]]
#     ###########
#     
#     tibble_joined_by_combined_name_and_ensembl_release_version[tibble_joined_by_combined_name_and_ensembl_release_version$gene_id.x %in% a1 | tibble_joined_by_combined_name_and_ensembl_release_version$gene_id.y %in% a1, ]
#     
...
tibble_HGNC_stable_protein_IDs <- furrr::future_imap(
  .x = list_tibble_all_ENSP_IDs_split_by_gene_name,
  .f = function(a1, a2) {
    
    # DEBUG ###
    # a2 <- list_tibble_all_ENSP_IDs_split_by_gene_name[[17819]]
    # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name[[76616]]
    # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name[[24]]
    # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name[[32]]
    # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name[[1]]
    # a1 <- list_tibble_all_ENSP_IDs_split_by_gene_name$`MT-ATP6`
    ###########
    ...
    # finally add in the stable transcript number
    tibble_hgnc_stable_protein_id <- tibble_with_ENSG_historical_join_order %>% 
      ungroup() %>% 
      group_split(gene_id) %>%
      purrr::imap(
        .x = .,
        .f = function(b1, b2) {
          ...
          output <- b1 %>% dplyr::arrange(protein_id)
      ...
      dplyr::ungroup() %>%
      dplyr::mutate("hgnc_stable_protein_ID" = paste(ensg_gene_name_combined, "-", ENSG_historical_join_order, "ensp", stable_protein_number, ".", protein_version, sep = "") %>% gsub(pattern = "\\-1ensp", replacement = "-ensp"))
