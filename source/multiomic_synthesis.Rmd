...
ensembl_archive_name <- "feb2021"
# ensembl_archive_name <- "sep2019"
# ensembl_archive_name <- "feb2014"
...
# retrieve uniprotkb_entry to ENSP mapping
tibble_uniprotkb_entry_to_ENSP_mapping <- dplyr::bind_rows(
  read.delim(file = paste(uniprot_dir, "idmapping.dat_ENSP", sep = ""), sep = "\t", stringsAsFactors = FALSE, check.names = FALSE, header = FALSE),
  read.delim(file = paste(uniprot_dir, "idmapping.dat.2015_03_ENSP", sep = ""), sep = "\t", stringsAsFactors = FALSE, check.names = FALSE, header = FALSE)
) %>% as_tibble %>% 
  setNames(c("uniprotkb_entry", "protein_id")) %>% unique %>% 
  dplyr::mutate(
...
tibble_uniprotkb_entry_to_ENSP_mapping[is.na(tibble_uniprotkb_entry_to_ENSP_mapping$uniprot_isoform_number), "uniprot_isoform_number"] <- 1
...
tibble_interpro_entry_mapping <- list_all_tibble_interpro_entries$`86.0`
...
list_biomart_features <- furrr::future_map(
  ...
  .progress = TRUE) %>%
  purrr::splice(
    "interpro" = furrr::future_map(
      .x = getBM(attributes = "chromosome_name", mart = ensembl_mart) %>% .$chromosome_name, 
      .f = function(a2) {
        getBM(attributes = c("ensembl_peptide_id", "ensembl_peptide_id_version", "interpro", "interpro_start", "interpro_end"), mart = ensembl_mart, filters = "chromosome_name", values = a2)
      } ) %>% rbindlist %>% 
      (function(x) {
        
        vector_ENSP_ids_missed <- setdiff(getBM(attributes = "ensembl_peptide_id", mart = ensembl_mart) %>% .$ensembl_peptide_id, x$ensembl_peptide_id)
        
        if (length(vector_ENSP_ids_missed) == 0) {
          return(x)
        } else {
          dplyr::bind_rows(
            x,
            getBM(attributes = c("ensembl_peptide_id", "ensembl_peptide_id_version", "interpro", "interpro_start", "interpro_end"), mart = ensembl_mart, filters = "ensembl_peptide_id", values = vector_ENSP_ids_missed)
          ) %>% return
        }
      } )
  )

list_biomart_features <- list_biomart_features %>% purrr::map(~.x %>% dplyr::rename("protein_id" = "ensembl_peptide_id") %>% dplyr::mutate("protein_version" = gsub(x = `ensembl_peptide_id_version`, pattern = ".*\\.(.*)$", replacement = "\\1") %>% type.convert) %>% dplyr::select(-ensembl_peptide_id_version) %>% na.omit)
  ...
  .f = function(a1, a2) {
    ...
    vec_all_genome_relative_coords_of_item <- future_map2(
      ...
      .f = function(b1, b2) {
        ...
        vec_parent_ENSP_CDS_positions[b1] %>% na.omit %>% return
    ...
    final_tibble_with_genome_relative_coords <- tibble_sorted_CDS_relative_positions %>% 
      tibble::add_column("vec_all_genome_relative_coords_of_item" = vec_all_genome_relative_coords_of_item,
                         "nucleotide_width" = nucleotide_width) %>% 
      dplyr::select(-vec_all_CDS_relative_positions, -vec_all_parent_ENSP_CDS_positions_sorted)
    
    return(final_tibble_with_genome_relative_coords)
    
  }, .progress = TRUE ) 

# add in the column type, then rbind and tibblise
tibble_biomart_regions_cast_onto_genome <- purrr::map2(
  .x = list_of_tibbles_biomart_regions_cast_onto_genome,
  .y = names(list_of_tibbles_biomart_regions_cast_onto_genome),
  .f = ~.x %>% tibble::add_column("region_type" = .y)
) %>% rbindlist(use.names = TRUE, fill = TRUE) %>% as_tibble

# sometimes there are incompatible ENSP IDs. they will result in 0 nucleotide width. remove these.
tibble_biomart_regions_cast_onto_genome <- tibble_biomart_regions_cast_onto_genome[tibble_biomart_regions_cast_onto_genome$nucleotide_width != 0, ]

# create GTF 
## sample an exon row from the GTF
# tibble_gtf_row_sample <- tibble_ref_gtf[tibble_ref_gtf$type == "exon", ] %>% .[1, ]

list_biomart_regions_cast_onto_genome_split <- tibble_biomart_regions_cast_onto_genome %>% dplyr::group_split(seqnames, region_type, ENTRY_TYPE)
    ...
    .f = function(a1, a2) {
      ...
      list_biomart_regions_gtf <- furrr::future_imap(
        ...
        .f = function(b1, b2) {
          ...
          if (length(vector_genome_relative_feature_positions) == 1) {
            
            vec_starts <- vector_genome_relative_feature_positions
            vec_ends <- vector_genome_relative_feature_positions
            vec_widths <- 1
          ...
          tibble_gtf_entries <- b1 %>%
            dplyr::select(-vec_all_genome_relative_coords_of_item, -nucleotide_width, -start, -end) %>%
            dplyr::bind_cols(
              .,
              tibble(
                "start" = vec_starts,
          ...
          # add exon number
          if (b1$strand == "+") {
            tibble_gtf_entries <- tibble_gtf_entries %>% dplyr::arrange(start) %>% tibble::add_column("exon_number" = 1:nrow(.))
          } else if (b1$strand == "-") {
            tibble_gtf_entries <- tibble_gtf_entries %>% dplyr::arrange(desc(start)) %>% tibble::add_column("exon_number" = 1:nrow(.))
          }
          
          return(tibble_gtf_entries)
...
tibble_biomart_regions_gtf0 <- list_biomart_regions_gtf %>% flatten %>% rbindlist %>% as_tibble

tibble_biomart_regions_gtf <- dplyr::left_join(tibble_biomart_regions_gtf0, tibble_uniprotkb_entry_to_ENSP_mapping) %>% 
  dplyr::left_join(., tibble_hgnc_stable_protein_id_mapping[tibble_hgnc_stable_protein_id_mapping$ensembl_release_version == tibble_hgnc_stable_protein_id_mapping$release_last_seen_protein_id, c("gene_id", "protein_id", "hgnc_stable_protein_ID")]) %>%
  dplyr::left_join(., tibble_uniprot_stable_protein_id_mapping) %>% 
  dplyr::mutate(
    "uniprot_stable_protein_ID" = paste(uniprotkb_id, "-u", uniprot_isoform_number, ".", uniprot_stable_protein_number, sep = "")
  )

tibble_biomart_regions_gtf[is.na(tibble_biomart_regions_gtf$uniprotkb_id), "uniprot_stable_protein_ID"] <- NA

data.table::fwrite(x = tibble_biomart_regions_gtf, file = paste(results_dir, "biomart_tracks_ensembl_98.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

## also write a gene-centric version with the uniprot_stable_protein_ID collapsed. We do this because it's a bit slow if we leave this until the app.
tibble_biomart_regions_gtf_gene_centric <- tibble_biomart_regions_gtf %>% dplyr::group_by(protein_id, protein_version, interpro, ENTRY_TYPE, ENTRY_NAME, transcript_id, transcript_version, seqnames, strand, region_type, start, end, width, type, exon_number, uniprotkb_entry, gene_id, hgnc_stable_protein_ID) %>% dplyr::summarise("uniprot_stable_protein_IDs" = `uniprot_stable_protein_ID` %>% paste(collapse = ";"))

## this is a bit more complicated because interpro is organised in a different way to the others
### add in the interpro names to the region_type column
tibble_biomart_regions_gtf_gene_centric[tibble_biomart_regions_gtf_gene_centric$region_type == "interpro", "region_type"] <- paste("interpro: ", tibble_biomart_regions_gtf_gene_centric[tibble_biomart_regions_gtf_gene_centric$region_type == "interpro", ] %>% .$ENTRY_NAME, sep = "")
### entry_type - those not interpro will be labelled "biomart"
tibble_biomart_regions_gtf_gene_centric$region_class <- tibble_biomart_regions_gtf_gene_centric$ENTRY_TYPE
tibble_biomart_regions_gtf_gene_centric[is.na(tibble_biomart_regions_gtf_gene_centric$region_class), "region_class"] <- "biomart"

## add id column because we're going to have to plot each region one by one
tibble_biomart_regions_gtf_gene_centric$id <- 1:nrow(tibble_biomart_regions_gtf_gene_centric)

data.table::fwrite(x = tibble_biomart_regions_gtf_gene_centric, file = paste(results_dir, "biomart_tracks_gene_centric_ensembl_98.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
...
tibble_dbPTM_allhuman_annotations_with_ENSP <- dplyr::left_join(tibble_dbPTM_with_modified_residue, tibble_uniprotkb_entry_to_ENSP_mapping) %>% 
  dplyr::left_join(., tibble_hgnc_stable_protein_id_mapping[tibble_hgnc_stable_protein_id_mapping$ensembl_release_version == tibble_hgnc_stable_protein_id_mapping$release_last_seen_protein_id, c("gene_id", "protein_id", "hgnc_stable_protein_ID")]) %>%
  dplyr::left_join(., tibble_uniprot_stable_protein_id_mapping) %>%
  dplyr::filter(is.na(protein_id) != TRUE | is.na(uniprotkb_id) != TRUE) %>% type_convert
...
tibble_dbPTM_allhuman_annotations_with_ENSP[tibble_dbPTM_allhuman_annotations_with_ENSP$PTM_type == "Crotonylation", "PTM_type"] <- "cr"
...
tibble_dbPTM_allhuman_annotations_with_ENSP[tibble_dbPTM_allhuman_annotations_with_ENSP$PTM_type %in% c("S-nitrosylation", "S-Nitrosylation"), "PTM_type"] <- "s-no"
...
tibble_dbPTM_allhuman_annotations_cast_onto_genome <- tibble_dbPTM_allhuman_annotations_with_ENSP %>% 
  ...
  # the parent ENSP CDS positions are sorted already. we just retrieve the genome-relative positions using the CDS-relative positions. 
  # these are the regions casted onto the genome.
  dplyr::mutate("vec_all_genome_relative_coords_of_item" = furrr::future_map2(.x = .$vec_all_CDS_relative_positions, .y = .$vec_all_parent_ENSP_CDS_positions_sorted, .f = ~.y %>% unlist %>% .[.x] %>% na.omit, .progress = TRUE)) %>%
...
tibble_dbPTM_allhuman_annotations_cast_onto_genome_subset <- tibble_dbPTM_allhuman_annotations_cast_onto_genome %>% dplyr::select(-CDS_relative_coords_start, -CDS_relative_coords_end, -vec_all_CDS_relative_positions, -vec_all_parent_ENSP_CDS_positions_sorted)

# create GTF 
## sample an exon row from the GTF
# tibble_gtf_row_sample <- tibble_ref_gtf[tibble_ref_gtf$type == "exon", ] %>% .[1, ]
list_dbPTM_allhuman_annotations_cast_onto_genome_split <- tibble_dbPTM_allhuman_annotations_cast_onto_genome_subset[tibble_dbPTM_allhuman_annotations_cast_onto_genome_subset$nucleotide_width > 0, ] %>% dplyr::group_split(seqnames, modified_residue, PTM_type) 

# %>% purrr::map(~.x[!is.na(.x$PTM_type), ])
...
suppressWarnings( suppressMessages( 
  ...
  list_dbPTM_regions_gtf <- furrr::future_imap(
    .x = list_dbPTM_allhuman_annotations_cast_onto_genome_split,
    .f = function(a1, a2) {
      
      # DEBUG ###
      # a1 <- list_dbPTM_allhuman_annotations_cast_onto_genome_split[[3]]
      ...
      list_dbPTM_regions_gtf <- furrr::future_imap(
        ...
        .f = function(b1, b2) {
          ...
          # DEBUG ###
          # b1 <- L1_split_list[[112]]
          
          # b1 <- list_dbPTM_allhuman_annotations_cast_onto_genome_split[[2]][112, ]
          ###########
          ...
          tibble_gtf_entries <- b1 %>%
            dplyr::select(-vec_all_genome_relative_coords_of_item) %>%
            dplyr::bind_cols(
              .,
              tibble(
                "start" = vec_starts,
      ...
      return(list_dbPTM_regions_gtf)
      
    }, .progress = TRUE )
  
) )

tibble_dbPTM_regions_gtf0 <- list_dbPTM_regions_gtf %>% flatten %>% rbindlist %>% as_tibble

# create uniprot_stable_protein_id
tibble_dbPTM_regions_gtf <- tibble_dbPTM_regions_gtf0 %>% 
  dplyr::mutate(
    "uniprot_stable_protein_ID" = paste(uniprotkb_id, "-u", uniprot_isoform_number, ".", uniprot_stable_protein_number, sep = "")
  )

# tibble_dbPTM_regions_gtf[is.na(tibble_dbPTM_regions_gtf$uniprotkb_id), "uniprot_stable_protein_ID"] <- NA

data.table::fwrite(x = tibble_dbPTM_regions_gtf, file = paste(results_dir, "dbPTM_tracks_ensembl_98.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)

# also write a gene-centric version with the uniprot_stable_protein_ID collapsed. We do this because it's a bit slow if we leave this until the app.
tibble_dbPTM_regions_gtf_gene_centric <- tibble_dbPTM_regions_gtf %>% dplyr::group_by(uniprotkb_entry, modified_residue_position, PTM_type, modified_residue, protein_id, gene_id, hgnc_stable_protein_ID, transcript_id, transcript_version, protein_version, seqnames, strand, nucleotide_width, start, end, width, type, exon_number) %>% dplyr::summarise("uniprot_stable_protein_IDs" = `uniprot_stable_protein_ID` %>% paste(collapse = ";"))

data.table::fwrite(x = tibble_dbPTM_regions_gtf_gene_centric, file = paste(results_dir, "dbPTM_tracks_gene_centric_ensembl_98.txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
