...
tibble_biomart_regions_gtf <- dplyr::left_join(
  tibble_hgnc_stable_protein_id_mapping[tibble_hgnc_stable_protein_id_mapping$ensembl_release_version == 98, ],
  tibble_biomart_regions_gtf0
) %>% 
  dplyr::left_join(., tibble_uniprotkb_entry_to_ENSP_mapping[grep(x = tibble_uniprotkb_entry_to_ENSP_mapping$protein_id, pattern = "ENSP\\d+"), ]) %>%
  dplyr::left_join(., tibble_uniprot_stable_protein_id_mapping) %>%
  dplyr::select(-first_release_ENSG_joined, -ENSG_historical_tree_order, -last_release_seen_gene_protein_id, -latest_ensg_gene_name_combined, -stable_protein_number, -release_last_seen_protein_id, -CDS_relative_coords_start, -CDS_relative_coords_end, -uniprotkb_id, -uniprot_stable_protein_number)
...
tibble_dbPTM_regions_gtf <- dplyr::left_join(
  tibble_hgnc_stable_protein_id_mapping[tibble_hgnc_stable_protein_id_mapping$ensembl_release_version == 98, ],
  tibble_dbPTM_regions_gtf0
) %>% 
  dplyr::left_join(., tibble_uniprotkb_entry_to_ENSP_mapping[grep(x = tibble_uniprotkb_entry_to_ENSP_mapping$protein_id, pattern = "ENSP\\d+"), ]) %>%
  dplyr::left_join(., tibble_uniprot_stable_protein_id_mapping) %>%
  dplyr::select(-first_release_ENSG_joined, -ENSG_historical_tree_order, -last_release_seen_gene_protein_id, -latest_ensg_gene_name_combined, -stable_protein_number, -release_last_seen_protein_id, -uniprotkb_id, -uniprot_stable_protein_number)
