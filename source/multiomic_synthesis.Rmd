...
results_dir <- "/mnt/LTS/projects/2020_isoform_nomenclature/results_genome_track_annotations/"
...
# Prepare Ensembl mappings

```{r}

View(listAttributes(ensembl_mart))
View(listFilters(ensembl_mart))

ensembl_archive_name

tibble_ENSP_to_uniprotkb_mapping <- biomaRt::getBM(attributes = c("ensembl_peptide_id", "uniprotsptrembl"), mart = ensembl_mart) %>% setNames(c("ensembl_peptide_id", "uniprotkb_entry")) %>%
  type_convert %>%
  as_tibble %>%
  na.omit
...
list_all_tibble_ref_gtf <- furrr::future_map(
  .x = list.files(path = ensembl_GTF_dump_dir, full.names = TRUE),
  .f = ~rtracklayer::import(con = .x) %>% as_tibble
) %>% set_names(nm = list.files(path = ensembl_GTF_dump_dir, full.names = FALSE))
...
list_all_tibble_ref_gtf_fixed <- furrr::future_map(
  .x = list_all_tibble_ref_gtf,
  .f = function(a1) {
    
    # DEBUG ###
    # a1 <- list_all_tibble_ref_gtf$Homo_sapiens.GRCh38.76.gtf
    ###########
...
save(list_all_tibble_ref_gtf_fixed, file = paste(results_dir, "list_all_tibble_ref_gtf_fixed.Rlist", sep = ""), compress = FALSE)

# 38.98 only
tibble_ref_gtf <- list_all_tibble_ref_gtf_fixed$Homo_sapiens.GRCh38.98.gtf

list_biomart_features <- furrr::future_map(
  .x = list(
    "interpro" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "interpro", "interpro_short_description", "interpro_start", "interpro_end"),
    "mobidblite" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "mobidblite_start", "mobidblite_end"),
    "ncoils" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "ncoils_start", "ncoils_end"),
    "seg" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "seg_start", "seg_end"),
    "signalp" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "signalp_start", "signalp_end"),
    "tmhmm" = c("ensembl_transcript_id", "ensembl_transcript_id_version", "ensembl_peptide_id", "ensembl_peptide_id_version", "tmhmm_start", "tmhmm_end")
  ),
